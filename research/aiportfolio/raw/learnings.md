...
- **Feb 22, 2026 (ALAB Lesson):** A stock can receive massive long-term fundamental PT upgrades (e.g., 2026 targets) but still break down technically (Stage 4) due to near-term earnings reset. Technicals trump fundamentals for entry timing; do not buy a falling knife just because 2-year targets are raised.
- **Mar 1, 2026 (Execution Lesson):** Brave search free tier is effectively 1 req/sec; parallel fan-out causes 429s and noisy aggregator-only coverage. For weekly reviews, run parallel sub-agents for charting/synthesis, but stagger news queries and prioritize primary sources (company IR, major sell-side notes, SEC forms) before using syndication summaries.
